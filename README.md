...
This repository is also structured for Codex-assisted OSS maintenance. See [Maintainer Workflows](docs/maintainer-workflows.md) for the PR review, issue triage, release validation, and regression-testing workflows where Codex/API credits would be used.
...
Agent Watch keeps generated notes, local SQLite state, and secrets out of git. The project includes CI, Dependabot, tests with local fixtures, source documentation, Obsidian setup notes, and a roadmap. For reviewers evaluating open-source maintenance use cases, the fastest path is:
